...
DEFAULT_CONFIG_NAMES = (Path("oxyseo.toml"), Path(".oxyseo") / "oxyseo.toml")
...
def _common_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--config", default=argparse.SUPPRESS, help="path to oxyseo.toml")
    return common
...
def _discover_config(args: argparse.Namespace) -> Path:
    ...
    for name in DEFAULT_CONFIG_NAMES:
        ...
        if candidate.exists():
            return candidate
    user_config = Path.home() / ".oxyseo" / "oxyseo.toml"
...
def _doctor(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "version": __version__,
        "checks": {
            "cli": {"ok": True, "binary": "oxyseo", "package": "oxyseo"},
            "config": config_check,
        },
    }
    if getattr(args, "json", False):
        ...
        print(f"oxyseo {__version__}: config {status}")
