...
#
# Output format:
#   project (branch) • Opus 4.6 • 2h15m 73% W6d12h 100% • $0.00 • CTX▼42%
#
...
#
# OAuth token: macOS Keychain, service "Claude Code-credentials", account "$(whoami)"
...
fetch_usage() {
    TOKEN=$(security find-generic-password -s "Claude Code-credentials" -a "$(whoami)" -w 2>/dev/null | python3 -c "
import sys, json
...
IFS=$'\t' read -r five_h_left week_left time_left week_days < <(
    python3 -c "
...
five_h_left = week_left = time_left = week_days = ''
...
print(f'{five_h_left}\t{week_left}\t{time_left}\t{week_days}')
